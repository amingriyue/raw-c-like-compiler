...
//打印语义错误
#define SEMERROR(code,name) printf("xxxxx") //Error::semError(code,name)
...
bool Var::setInit()
{
	...
	else if(false)//!GenIR::typeCheck(this,init))
...
/*
	声明定义匹配
*/
#define SEMWARN(code,name) printf("warning\n") // Error::semWarn(code,name)
bool Fun::match(Fun*f)
{
	...
	for(int i=0;i<len;i++){
		if(false) {//GenIR::typeCheck(paraVar[i],f->paraVar[i])){//类型兼容
...
bool Fun::match(vector<Var*>&args)
{
	...
	int len=paraVar.size();
	for(int i=0;i<len;i++){
		if(false)//!GenIR::typeCheck(paraVar[i],args[i]))//类型检查不兼容
			return false;
	}
	return true;		
}
...
void Fun::genIr(FILE*file)
{
	...
	{
		fprintf(file, "%s", code[i]->InstToStr().c_str());
	}
